...
export function Component() {
  ...
  const handlePlayerReady = function (player: Player): void {
    ...
    player.on("timeupdate", () => {
      setPlayerState((draft) => {
        return { ...draft, playedSeconds: player.currentTime() || 0 }; // Handle undefined by providing a default value
      });
    });
  ...
  //   });
  // };
  const handleVolumeChange: SliderUnstyledOwnProps["onChange"] = ( _ : any, value: number) => {
    playerRef.current?.volume((value as number) / 100);
    setPlayerState((draft) => {
        return { ...draft, volume: (value as number) / 100 };
    });
